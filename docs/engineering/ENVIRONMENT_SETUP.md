...
worktree: в нём создаётся runtime profile Godot, а за 254 символами движок
перестаёт входить в собственный shader cache — см. [«Длина пути профиля Godot и
shader cache»](#длина-пути-профиля-godot-и-shader-cache). Подставить
`.artifacts/` вместо системного temp — ровно тот случай, на котором открылся
Issue #184: `DUNGEON_FORTRESS_TEMP=.artifacts/tmp` из worktree дал `APPDATA`
длиной 110 символов и 14 строк ошибки. Прогон по-прежнему отказывается угадывать
каталог, а теперь ещё и **измеряет** выбранный: слишком глубокий отвергается по
имени до запуска движка, а не молча.
