...
\*\* Строка `scripts` перемерена на машине агента, которая примерно вдвое
медленнее машины владельца: те же прежние гварды дают на ней около 12 с из этих
25 с. С остальными строками таблицы это число напрямую не сравнивается. Оно
росло дважды — на восьмом гварде и на разборе дот-сорсенных файлов из Issue
#102.
