...
Оболочка агента — не оболочка владельца, и расходится она в одном обязательном
месте плюс одном, который был обязательным до Issue #302 и больше не является
таковым:
...
**Движок — обязателен.** `GODOT4_CONSOLE` задан в пользовательском окружении
владельца и оболочкой агента **не наследуется**, поэтому `-GodotPath`
обязателен. Порядок разрешения движка описан в «Обязательных компонентах» и не
меняется: явный параметр, затем переменная, затем `PATH`. Агенту доступен
только первый.
...
в документ, — поэтому обязательность `-GodotPath` записана здесь требованием,
а не примечанием.
