...
(`~/.claude/projects/...`). На машине агента `.\scripts\verify.ps1 -Stage
token-budget` показал 824,1 с при первом прогоне за сессию
(`evidence/310-full-verify-in-scripts-stage.json`, элемент `scripts` до
выноса теста из этой стадии) и 115,1 с несколько прогонов спустя в той же
сессии (`evidence/310-token-budget-stage.json`), без изменений в самом
тесте. Разброс измерен и воспроизводим; его причина — не установленный
факт, а предположение: наиболее вероятное объяснение — состояние файлового
кэша ОС (директория к этому моменту уже читалась несколько раз подряд),
но изолированный опыт «холодный кэш против прогретого» не ставился, и
другие причины не исключены. Именно поэтому тест не годится ни одной другой
стадии, кроме собственной — время его прогона непредсказуемо независимо от
объяснения, а держать его отдельно от `scripts` не даёт этой
непредсказуемости заражать более дешёвые проверки.
...
по одной строке `verification_stage` на стадию (статус и время), `verification_result_file`
(см. «Результат прогона переживает уборку» ниже) и итоговый
...
### Результат прогона переживает уборку

Issue #427. Итоговая строка `verification_result` — четыре контрольные суммы
(`deterministicChecksum`, `changedSeedChecksum`, `loadChecksum`,
`viewInvariantChecksum`) и всё остальное, что называет полный прогон — до этой
правки печаталась только в stdout, а `finally`-блок безусловно удалял
`$verifyRoot`, где лежал `stage-output.log`, на любом исходе — зелёном и
красном. Числа не переживали сам прогон: PR #425 сослался на четыре
контрольные суммы, а `evidence/423-verify.json` их не содержал, потому что
каталог с первым прогоном был уже удалён, а память сессии источником не
является — цена оказалась в **втором полном прогоне** только ради того, чтобы
поймать те же числа через `Tee-Object`.

Уборка временного каталога не отменяется — она остаётся тем же best-effort
правилом (см. «Временный каталог» выше и «Прогон не оставляет следов в среде
машины»). Меняется только то, что `verification_result` (и, на красном
прогоне, `stage-output.log`) теперь пишутся ещё и в отдельный, не временный
файл, прежде чем `finally` удалит каталог, где они изначально появились.
Путь фиксированный и не содержит GUID — каждый прогон **перезаписывает**, а не
накапливает:

```
.artifacts/verify-results/verify-result.json
.artifacts/verify-results/stage-output.log   # только на красном прогоне
```

Путь называет сам скрипт, отдельной строкой перед итоговой:
...
`verification_result`, а открывать лог на зелёном прогоне незачем — та же
причина, по которой Issue #284 не держит его в stdout. На красном прогоне лог
копируется из `$verifyRoot` в `.artifacts/verify-results/stage-output.log`
...
же директория, где между прогонами уже живёт `$env:DOTNET_CLI_HOME` и
NuGet tool profile движка: файл результата не заводит новое исключение из
правила «не оставлять следов», а продолжает существующую конвенцию — он
живёт внутри рабочей копии, не в системном временном каталоге и не в среде
пользователя, и не накапливается, потому что каждый прогон его перезаписывает.

Четыре контрольные суммы читаются из файла одной командой, без повторного
прогона:

```powershell
(Get-Content .artifacts/verify-results/verify-result.json | ConvertFrom-Json) |
    Select-Object deterministicChecksum, changedSeedChecksum, loadChecksum, viewInvariantChecksum
```

Реализация — `scripts/VerifyResult.ps1` (`Save-VerificationResult`), её
дерево-независимый тест — `scripts/test-verify-result-persistence.ps1`,
подключённый в стадию `scripts`.
...
копию координационной сессии: ветку в ней не переключает и не коммитит из
неё никто — ни записывающий агент любого инструмента, ни сама
координационная сессия для своей же записывающей работы. За четыре дня
правило нарушено трижды тремя разными
