...
Поскольку утверждение engine-free, его можно вызвать и из обычного юнит-теста
без движка — `CameraView.AssertStartupFramePolicy(ViewLaunchOptions.MinimumLogicalFrameSize)`.
Сейчас его вызывает Godot-адаптер на каждом входе.

#### Что из Issue #86 осталось

Наблюдаемый дефект #86 закрыт: HUD на максимизированном окне большого экрана
читается. Не сделано и остаётся за #86 два пункта, оба вне областей этой задачи:
