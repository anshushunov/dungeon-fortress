...
сборка. Копия снимается на старте сессии, поэтому свежесобранный домен через MCP
видит только новая сессия — после правок ядра сессию нужно перезапустить. Схема,
её причина и автоматические проверки описаны в
...
размеченную клетку, команда, назначенная на более поздний тик, заранее не
показывается, а команда, которая ничего не изменит, не считается ожидающей —
как и в самой симуляции. Набор `MapAccentTests` закрывает вторую половину того
же критерия: он сравнивает не множество клеток, а **прочтение** каждой метки —
ожидающее против того, которое даёт мир, — прогоняя настоящую симуляцию через
применяющий тик, и отдельно требует, чтобы старая и новая метки в одном кадре
читались одинаково — и при ожидающей смене приоритета, и при ожидающей закраске
`Forbidden`, в обе стороны. Граница сформулирована правилом, а не списком
исключений: проекция отвечает на то, что выводится из опубликованных фактов,
свёрнутых через неё, а мир — на то, что требует прогона тика. Полный перечень
фактов из лестниц статусов, с вердиктом по каждому — «свёрнут», «невозможен как
ожидающий» или «принадлежит миру», — лежит в
[`PROTOTYPE_GRAYBOX.md`](PROTOTYPE_GRAYBOX.md#where-the-line-is) и в docstring
`MapAccents`. Приоритеты и `Forbidden` свёрнуты; достижимость породы, начало
работы, выбор исполнителя и перемещение материала остаются за миром. Лестницы
повторены в одном месте и запинены свипом целой сессии: предсказание сверяется с
`statusCode` мира на каждом тике, где ничего не ожидает. Отдельный набор
воспроизводит три эталонных кадра
