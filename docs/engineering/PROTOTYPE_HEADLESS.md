...
читались одинаково. Граница сформулирована правилом, а не списком исключений:
проекция отвечает на то, что выводится из опубликованных фактов, свёрнутых через
неё, а мир — на то, что требует прогона тика. Поэтому приоритет работ входит в
проекцию (он ничего не рисует, но стоит первой ступенью лестницы статусов), а
достижимость породы, начало работы и выбор исполнителя остаются за миром.
Лестница строительства повторена в одном месте и запинена свипом целой сессии:
предсказание сверяется с `statusCode` мира на каждом тике. Отдельный набор
