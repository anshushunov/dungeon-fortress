...
применяющий тик. Меняются ровно два прочтения, и оба названы: `dig_unreachable`
(достижимость — это топология карты, копировать её в представление запрещает ADR
0011) и `dig_in_progress` (существо начало копать). Приоритет работ входит в
проекцию по той же причине: он ничего не рисует, но стоит первой ступенью в
лестнице статусов мира, поэтому «выключить копку и тут же разметить» — один жест
и одно прочтение. Отдельный набор воспроизводит три эталонных кадра
