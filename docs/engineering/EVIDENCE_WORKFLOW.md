...
- `not-runnable` — команда не запущена, потому что у неё нет claim, она не
  проходит allow-list или завершилась с ненулевым кодом.
...
Скрипт исполняет команды из тела PR — текст, написанный другим агентом. Поэтому
он работает по allow-list по существу, а не по deny-list по именам: каждая
команда разбирается на токены, и запускается только команда, целиком
соответствующая одному из двух разрешённых шаблонов.

Разрешены только:

- `powershell` / `pwsh` с флагами `-NoProfile`, `-NoLogo`, `-NonInteractive`,
  `-STA`, `-ExecutionPolicy <значение>` и ровно одним `-File <путь>` — запуск
  локального скрипта из рабочей копии. Путь обязан резолвиться внутри `-RepoRoot`;
- `git` с read-only подкомандой: `status`, `log`, `show`, `diff`, `rev-parse`,
  `describe`, `ls-files`, `ls-tree`, `cat-file`, `blame`, `shortlog`, `grep`,
  `count-objects`, `for-each-ref`, `show-ref`, `name-rev`, `check-attr`,
  `check-ignore`, `check-mailmap`. Аргументы с `--output` или shell-метасимволами
  (`` ` ``, `;`, `|`, `&`, `<`, `>`) не допускаются.

Запрещены любые формы инлайнового исполнения: `-Command`, `-c`,
`-EncodedCommand`, `-e`, а также любой другой исполняемый файл. Deny-list по
именам не используется, потому что его можно обойти алиасами (`rm`, `ri`,
`del`), .NET-рефлексией (`[System.IO.File]::Delete`, `WebClient`),
`Invoke-Expression` и мутирующими git-подкомандами вне списка (`git branch -D`,
`git push`).

Сбой команды не роняет скрипт и не глушит остальные блоки тела: каждая пара
обрабатывается независимо, и команда, которую не удалось запустить или которая
завершилась с ненулевым кодом, помечается `not-runnable` с причиной.
