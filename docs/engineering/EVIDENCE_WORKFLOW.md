...
Заявленные в `evidence/*.json` и `docs/art/*.md` SHA-256 сверяются с
**коммиченным** состоянием дерева, а не с рабочей копией:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\check-claimed-sha256.ps1
```

Скрипт разбирает пары `*Path`/`*Sha256` в evidence JSON и строки `docs/art` и для
каждой заявленной суммы сообщает: совпадает с блобом (`blob-match`), совпадает
только с рабочей копией (`working-copy-only` — капкан переводов строк), не
совпадает ни с чем (`mismatch`) или файл не в дереве (`untracked`). Ненулевой exit
код при `mismatch` или `working-copy-only` для отслеживаемого файла.
