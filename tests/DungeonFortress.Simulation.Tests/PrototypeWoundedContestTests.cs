            ...
            JsonSerializer.Serialize(
                new
                {
                    ...
                    formula =
                        ...
                        "разделяя его.",
                    // Записан здесь, в генераторе, а не дописан в файл руками:
                    // всё, что дописано в сгенерированный файл, стирается первым
                    // же прогоном — это уже случилось один раз с этим блоком.
                    theMutant = new
                    {
                        what =
                            "M-ORDER: перенос проверки достижимости ЗА блок состязания в " +
                            "UpdateCombatParticipation — тот самый порядок, который правило " +
                            "запрещает.",
                        command =
                            "механический перенос блока в " +
                            "src/DungeonFortress.Simulation/PrototypeWorld.Combat.cs, прогон БЕЗ " +
                            "--no-build, затем git checkout -- и rg -n \"MUTANT\"",
                        prediction =
                            "Свидетель исчезает: Прель доходит до состязания раньше проверки " +
                            "расстояния, пишет combat_spared_wound и делает continue, поэтому " +
                            "кода недосягаемости не получает вовсе.",
                        observed =
                            "Свидетелей 1 -> 0; отказов по недосягаемости 26 -> 25, из них " +
                            "раненых 14 -> 13. Дополнительно всплывают столкновения «и " +
                            "недосягаем, и pressed на одной перекличке» на четырёх партиях " +
                            "матрицы.",
                        reading =
                            "Проверка чувствительна ДВУМЯ независимыми утверждениями: классовым " +
                            "(никто никогда не является тем и другим сразу) и позитивным " +
                            "(свидетель существует). Второе — то, чего у проверки не было до " +
                            "находки 3 независимого review PR #436: без свидетеля позитивная " +
                            "ветка не имела предмета, и зелёный цвет ничего не значил.",
                    },
                },
